...
def sum_of_angle(angle1, angle2):
    r = angle1 + angle2
    return r if (r <= 360) else (r - 360.0)
...
def reward_function(params):
    ...
    heading_angle           = diff_of_angle(params["heading"], angle_of_vector([waypoints[pre], waypoints[nxt]])) # 航向-道路夹角
    ...
    if is_offtrack:
        ...
        a1 = 1.0
        if reduce(lambda x, y: x + y, forward_directions) == forward_directions[0] * len(forward_directions): # 直道
            a = 100*math.cos(math.pi/2*min(sum_of_angle(heading_angle, steering_angle), 90)/90)-99
            if a < 0:
                return a
            a1 = B * a + 1
        ...
        rwd = a1 * s1 * w * t
        print("R=%s (a1:%s, s1:%s, w:%s, t:%s) - is_offtrack:%s, speed:%s, progress:%s, all_wheels_on_track:%s, distance_from_center:%s, steps:%s, track_width:%s, heading_direction:%s, steering_angle:%s, heading_angle:%s, forward_waypoints:%s, forward_directions:%s" % (rwd, a1, s1, w, t, is_offtrack,speed,progress,all_wheels_on_track,distance_from_center,steps,track_width,heading_direction,steering_angle,heading_angle,forward_waypoints,forward_directions))
        return rwd
